...
void sensor_thread_entry(void *pvParameters) {
  ...
  uint8_t icp_data[20] = {0};
  uint8_t zmod_data[20] = {0};
  char uart_msg[128];
  ...
  while (1) {
    snprintf(uart_msg, sizeof(uart_msg), "Start Reading! \n");
    RM_COMMS_UART_Write(&g_comms_uart0_ctrl, (uint8_t *)uart_msg,
                        strlen(uart_msg));

    // --- Read ICP-10101 ---
    // fsp_err_t read_icp =
    //     RM_COMMS_I2C_Read(&g_icp_comms_i2c_ctrl, icp_data, sizeof(icp_data));
    // // while (!i2c_flag) { /* Wait for callback! */
    // // }
    // // i2c_flag = 0;

    // --- Read ZMOD4410 ---
    // fsp_err_t read_zmod =
    //     RM_COMMS_I2C_Read(&g_zmod_comms_i2c_ctrl, zmod_data, sizeof(zmod_data));
    // // while (!i2c_flag) { /* Wait for callback! */
    // // }
    // // i2c_flag = 0;
    ...
    // // --- Process data: (quick raw parsing, actual implementation may need
    // // sensor libraries to decode IAQ, pressure) ---
...
void icp_comms_i2c_callback(rm_comms_callback_args_t *p_args) {
  if (p_args->event == RM_COMMS_EVENT_RX_OPERATION_COMPLETE) {
    i2c_flag = 1;
  }
}
