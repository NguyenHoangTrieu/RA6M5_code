...
/* Definition of the maximum / minimum number of data that can be sent at one
 * time in the Ethernet */
#define ETHER_MAXIMUM_FRAME_SIZE                                               \
  (1514U)                              /* Maximum number of transmitted data */
...
#define ETHER_PAUSE_XMIT_OFF                                                   \
  (0) /* The pause frame transmission is prohibited. */
#define ETHER_PAUSE_RECV_OFF                                                   \
  (0) /* The pause frame reception is prohibited.    */
#define ETHER_PAUSE_XMIT_ON                                                    \
  (1) /* The pause frame transmission is permitted.  */
#define ETHER_PAUSE_RECV_ON                                                    \
  (1) /* The pause frame reception is permitted.     */

/* Macro definition for buffer alignment adjustment */
#define ETHER_BUFFER_32BYTE_ALIGNMENT_MASK                                     \
  (0x1FUL) /* Mask for checking whether or not 32-bit alignment. */

/* PAUSE link mask and shift values */

/*
 * The mask value and shift value which are for that shift the bits form a line
 * and for comparing the bit information of PAUSE function which support the
 * local device and Link partner with the assorted table(pause_resolution) which
 * enable or disable the PAUSE frame.
 */
...
#define ETHER_EESR_ERR_GLOBAL_MASK                                             \
...
typedef void(BSP_CMSE_NONSECURE_CALL *ether_prv_ns_callback)(
    ether_callback_args_t *p_args);
#elif defined(__GNUC__)
typedef BSP_CMSE_NONSECURE_CALL void (*volatile ether_prv_ns_callback)(
    ether_callback_args_t *p_args);
#endif
...
static fsp_err_t
ether_open_param_check(ether_instance_ctrl_t const *const p_instance_ctrl,
...
static void
ether_init_descriptors(ether_instance_ctrl_t *const p_instance_ctrl);
...
static void
ether_config_ethernet(ether_instance_ctrl_t const *const p_instance_ctrl,
                      const uint8_t mode);
static void ether_pause_resolution(uint32_t const local_ability,
                                   uint32_t const partner_ability,
                                   uint32_t *ptx_pause, uint32_t *prx_pause);
static void ether_configure_mac(ether_instance_ctrl_t *const p_instance_ctrl,
                                const uint8_t mac_addr[], const uint8_t mode);
static fsp_err_t ether_do_link(ether_instance_ctrl_t *const p_instance_ctrl,
                               const uint8_t mode);
static fsp_err_t
ether_link_status_check(ether_instance_ctrl_t const *const p_instance_ctrl);
static uint8_t ether_check_magic_packet_detection_bit(
    ether_instance_ctrl_t const *const p_instance_ctrl);
static void
ether_configure_padding(ether_instance_ctrl_t *const p_instance_ctrl);
static void ether_call_callback(ether_instance_ctrl_t *p_instance_ctrl,
...
/* This structure is affected by warnings from a GCC compiler bug. This pragma
 * suppresses the warnings in this structure only.*/
 ...
 */
static const ether_pause_resolution_t
    pause_resolution[ETHER_PAUSE_TABLE_ENTRIES] = {
        {ETHER_PAUSE_MASKC, ETHER_PAUSE_VAL0, ETHER_PAUSE_XMIT_OFF,
         ETHER_PAUSE_RECV_OFF},
        {ETHER_PAUSE_MASKE, ETHER_PAUSE_VAL4, ETHER_PAUSE_XMIT_OFF,
         ETHER_PAUSE_RECV_OFF},
        {ETHER_PAUSE_MASKF, ETHER_PAUSE_VAL6, ETHER_PAUSE_XMIT_OFF,
         ETHER_PAUSE_RECV_OFF},
        {ETHER_PAUSE_MASKF, ETHER_PAUSE_VAL7, ETHER_PAUSE_XMIT_ON,
         ETHER_PAUSE_RECV_OFF},
        {ETHER_PAUSE_MASKE, ETHER_PAUSE_VAL8, ETHER_PAUSE_XMIT_OFF,
         ETHER_PAUSE_RECV_OFF},
        {ETHER_PAUSE_MASKA, ETHER_PAUSE_VALA, ETHER_PAUSE_XMIT_ON,
         ETHER_PAUSE_RECV_ON},
        {ETHER_PAUSE_MASKF, ETHER_PAUSE_VALC, ETHER_PAUSE_XMIT_OFF,
         ETHER_PAUSE_RECV_OFF},
        {ETHER_PAUSE_MASKF, ETHER_PAUSE_VALD, ETHER_PAUSE_XMIT_OFF,
         ETHER_PAUSE_RECV_ON}};
static const ether_event_mask_t ether_eesr_event_mask[ETHER_EESR_EVENT_NUM] = {
    ...
    {.event = ETHER_EVENT_RX_MESSAGE_LOST,
     .mask = ETHER_EESR_RX_MESSAGE_LOST_MASK},
    {.event = ETHER_EVENT_TX_COMPLETE, .mask = ETHER_EESR_TX_COMPLETE_MASK},
    {.event = ETHER_EVENT_TX_BUFFER_EMPTY,
     .mask = ETHER_EESR_TX_BUFFER_EMPTY_MASK},
...
fsp_err_t R_ETHER_Open(ether_ctrl_t *const p_ctrl,
...
#endif
  ETHER_ERROR_RETURN((ETHER_OPEN != p_instance_ctrl->open),
                     FSP_ERR_ALREADY_OPEN);

  p_ether_extended_cfg = (ether_extended_cfg_t *)p_cfg->p_extend;

  /** Make sure this channel exists. */
  p_instance_ctrl->p_reg_etherc =
      ((R_ETHERC0_Type *)(R_ETHERC0_BASE +
                          (ETHER_ETHERC_REG_SIZE * p_cfg->channel)));
  p_instance_ctrl->p_reg_edmac =
      ((R_ETHERC_EDMAC_Type *)(R_ETHERC_EDMAC_BASE +
                               (ETHER_ETHERC_EDMAC_REG_SIZE * p_cfg->channel)));
  ...
  if (FSP_SUCCESS == phy_ret) {
    phy_ret =
        p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_api->chipInit(
            p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_ctrl,
            p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_cfg);
  }
#endif
  ...
  if (FSP_SUCCESS == phy_ret) {
    p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_api
        ->startAutoNegotiate(
            p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_ctrl);
...
fsp_err_t R_ETHER_RxBufferUpdate(ether_ctrl_t *const p_ctrl,
  ...
  ETHER_ERROR_RETURN(ETHER_ZEROCOPY_ENABLE ==
                         p_instance_ctrl->p_ether_cfg->zerocopy,
...
fsp_err_t R_ETHER_LinkProcess(ether_ctrl_t *const p_ctrl) {
  ...
  ETHER_ERROR_RETURN(ETHER_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
#endif
  p_ether_extended_cfg =
      (ether_extended_cfg_t *)p_instance_ctrl->p_ether_cfg->p_extend;
  ...
  if (ETHER_MAGIC_PACKET_DETECTED == p_instance_ctrl->magic_packet) {
    ...
    err = R_ETHER_Open((ether_ctrl_t *)p_instance_ctrl,
                       (ether_cfg_t *)p_ether_cfg);
    ETHER_ERROR_RETURN(FSP_SUCCESS == err, err);
  ...
  /* The state of the link status in PHY-LSI is confirmed and Link Up/Down is
   * judged. */
  if (FSP_SUCCESS == err) {
    /* When becoming Link up */
    if (ETHER_PREVIOUS_LINK_STATUS_DOWN ==
        p_instance_ctrl->previous_link_status) {
    ...
    if (ETHER_PREVIOUS_LINK_STATUS_UP ==
        p_instance_ctrl->previous_link_status) {
  ...
  if (ETHER_LINK_CHANGE_LINK_UP == p_instance_ctrl->link_change) {
    ...
    if (FSP_SUCCESS == err) {
      ...
      /*
       * The status of the LINK signal became "link-up" even if PHY-LSI did not
       * detect "link-up" after a reset. To avoid this wrong detection,
       * processing in R_ETHER_LinkProcess has been modified to clear the flag
       * after link-up is confirmed in R_ETHER_CheckLink_ZC.
       */
#if (ETHER_CFG_USE_LINKSTA == 1)
      p_instance_ctrl->link_change = ETHER_LINK_CHANGE_LINK_DOWN;
...
/********************************************************************************************************************/ /**
                                                                                                                        ...
                                                                                                                        *
                                                                                                                        ***********************************************************************************************************************/
fsp_err_t R_ETHER_Read(ether_ctrl_t *const p_ctrl, void *const p_buffer,
                       uint32_t *const length_bytes) {
  fsp_err_t err = FSP_SUCCESS;
  ether_instance_ctrl_t *p_instance_ctrl = (ether_instance_ctrl_t *)p_ctrl;
  uint8_t *p_read_buffer =
      NULL; /* Buffer location controlled by the Ethernet driver */
  ...
  while (FSP_SUCCESS == err) {
    ...
    if (ETHER_RD0_RACT !=
      ...
      if (ETHER_MULTICAST_DISABLE == p_instance_ctrl->p_ether_cfg->multicast) {
        if (ETHER_RD0_RFS7_RMAF ==
            (p_instance_ctrl->p_rx_descriptor->status & ETHER_RD0_RFS7_RMAF)) {
      ...
      if (ETHER_RD0_RFE ==
          (p_instance_ctrl->p_rx_descriptor->status & ETHER_RD0_RFE)) {
  ...
  if (FSP_SUCCESS == err) {
    ...
    if (received_size > ETHER_NO_DATA) {
      if (ETHER_ZEROCOPY_DISABLE == p_instance_ctrl->p_ether_cfg->zerocopy) {
         ...
         *     by the descriptor to the buffer which is specified by the user
         * (up to 1024 bytes). */
        memcpy(p_buffer, p_read_buffer, received_size);

        /* (3) Read the receive data from the receive buffer controlled by the
         * descriptor, and then release the receive buffer. */
        err = R_ETHER_BufferRelease((ether_ctrl_t *)p_instance_ctrl);
      } else {
...
/********************************************************************************************************************/ /**
                                                                                                                        ...
                                                                                                                        *
                                                                                                                        ***********************************************************************************************************************/
fsp_err_t R_ETHER_Write(ether_ctrl_t *const p_ctrl, void *const p_buffer,
  ...
  ETHER_ERROR_RETURN(ETHER_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
  ETHER_ERROR_RETURN(NULL != p_buffer, FSP_ERR_INVALID_POINTER);
  ETHER_ERROR_RETURN((ETHER_MINIMUM_FRAME_SIZE <= frame_length) &&
                         (ETHER_MAXIMUM_FRAME_SIZE >= frame_length),
  ...
  if (ETHER_ZEROCOPY_DISABLE == p_instance_ctrl->p_ether_cfg->zerocopy) {
     ...
     */
    err = ether_buffer_get(p_instance_ctrl, (void **)&p_write_buffer,
                           &write_buffer_size);

    /* Writing to the transmit buffer (buf) is enabled. */
    if (FSP_SUCCESS == err) {
      if (write_buffer_size < frame_length) {
        err = FSP_ERR_ETHER_ERROR_TRANSMIT_BUFFER_FULL; /* Transmit buffer
                                                           overflow */
      } else {
        /* Write the transmit data to the transmit buffer. */

        /* (2) Write the data to the transmit buffer controlled by the
         * descriptor. */
  ...
  if (FSP_SUCCESS == err) {
    ...
    p_instance_ctrl->p_tx_descriptor->status &=
        (~(ETHER_TD0_TFP1 | ETHER_TD0_TFP0));
    p_instance_ctrl->p_tx_descriptor->status |=
        ((ETHER_TD0_TFP1 | ETHER_TD0_TFP0) | ETHER_TD0_TACT);
...
fsp_err_t R_ETHER_TxStatusGet(ether_ctrl_t *const p_ctrl,
  ...
  if (NULL != p_descriptor) {
    ...
    ether_instance_descriptor_t *p_tx_descriptors =
        p_ether_extended_cfg->p_tx_descriptors;

    p_descriptor =
        (ether_instance_descriptor_t *)((uint8_t *)p_descriptor -
                                        sizeof(ether_instance_descriptor_t));
    ...
    if ((NULL != p_descriptor->p_buffer) &&
        (ETHER_TD0_TACT != (p_descriptor->status & ETHER_TD0_TACT))) {
...
fsp_err_t R_ETHER_CallbackSet(ether_ctrl_t *const p_api_ctrl,
  ...
  bool callback_is_secure =
      (NULL == cmse_check_address_range((void *)p_callback, sizeof(void *),
                                        CMSE_AU_NONSECURE));

#if ETHER_CFG_PARAM_CHECKING_ENABLE

  /* In secure projects, p_callback_memory must be provided in non-secure space
   * if p_callback is non-secure */
  ether_callback_args_t *const p_callback_memory_checked =
      cmse_check_pointed_object(p_callback_memory, CMSE_AU_NONSECURE);
  FSP_ERROR_RETURN(callback_is_secure || (NULL != p_callback_memory_checked),
...
ether_open_param_check(ether_instance_ctrl_t const *const p_instance_ctrl,
  ...
  ETHER_ERROR_RETURN((BSP_FEATURE_ETHER_NUM_CHANNELS > p_cfg->channel),
                     FSP_ERR_INVALID_CHANNEL);
  ETHER_ERROR_RETURN((0 <= p_cfg->irq), FSP_ERR_INVALID_ARGUMENT);
  ETHER_ERROR_RETURN((p_cfg->padding <= ETHER_PADDING_3BYTE),
                     FSP_ERR_INVALID_ARGUMENT);
  ...
  if (p_cfg->zerocopy == ETHER_ZEROCOPY_DISABLE) {
    ETHER_ERROR_RETURN((p_cfg->pp_ether_buffers != NULL),
                       FSP_ERR_INVALID_ARGUMENT);
  }
...
static void ether_reset_mac(R_ETHERC_EDMAC_Type *const p_reg) {
  ...
  /*
   * Waiting time until the initialization of ETHERC and EDMAC is completed is
   * 64 cycles in the clock conversion of an internal bus of EDMAC.
   */
  R_BSP_SoftwareDelay(ETHER_ETHERC_INITIALIZATION_WAIT_CYCLE *
...
ether_init_descriptors(ether_instance_ctrl_t *const p_instance_ctrl) {
  ...
  for (i = 0; i < p_instance_ctrl->p_ether_cfg->num_rx_descriptors; i++) {
    ...
    p_descriptor->buffer_size =
        (uint16_t)p_instance_ctrl->p_ether_cfg->ether_buffer_size;
    p_descriptor->size = 0;
    ...
    if (NULL != p_instance_ctrl->p_ether_cfg->pp_ether_buffers) {
      p_descriptor->p_buffer =
          p_instance_ctrl->p_ether_cfg->pp_ether_buffers[i];
  ...
  if (NULL != p_descriptor) {
    ...
    p_instance_ctrl->p_rx_descriptor =
        &p_ether_extended_cfg->p_rx_descriptors[0];
  }
  ...
  for (i = 0; i < p_instance_ctrl->p_ether_cfg->num_tx_descriptors; i++) {
    ...
    p_descriptor->buffer_size =
        1; /* Set a value equal to or greater than 1. (reference to UMH)
            * When transmitting data, the value of size is set to the function
            * argument R_ETHER_Write. */
    p_descriptor->size =
        0; /* Reserved : The write value should be 0. (reference to UMH) */
    ...
    if ((ETHER_ZEROCOPY_DISABLE == p_instance_ctrl->p_ether_cfg->zerocopy) &&
      ...
      p_descriptor->p_buffer = p_instance_ctrl->p_ether_cfg->pp_ether_buffers[(
          p_instance_ctrl->p_ether_cfg->num_rx_descriptors + i)];
    } else {
      p_descriptor->p_buffer = NULL;
    }
  }
  ...
  if (NULL != p_descriptor) {
    ...
    p_instance_ctrl->p_tx_descriptor =
        &p_ether_extended_cfg->p_tx_descriptors[0];
  }
...
static void ether_init_buffers(ether_instance_ctrl_t *const p_instance_ctrl) {
  ...
  if (NULL != p_instance_ctrl->p_ether_cfg->pp_ether_buffers) {
    if (ETHER_ZEROCOPY_DISABLE == p_instance_ctrl->p_ether_cfg->zerocopy) {
      buffer_num = (uint32_t)(p_instance_ctrl->p_ether_cfg->num_tx_descriptors +
                              p_instance_ctrl->p_ether_cfg->num_rx_descriptors);
    } else {
...
static fsp_err_t ether_buffer_get(ether_instance_ctrl_t *const p_instance_ctrl,
  ...
  if (ETHER_TD0_TACT ==
      (p_instance_ctrl->p_tx_descriptor->status & ETHER_TD0_TACT)) {
 ...
 *                   USE_MAGIC_PACKET_DETECT    (1) - Magic packet detection
...
ether_config_ethernet(ether_instance_ctrl_t const *const p_instance_ctrl,
...
#if (ETHER_CFG_USE_LINKSTA == 1)
    p_reg_etherc->ECSIPR = (ETHER_ETHERC_INTERRUPT_ENABLE_LCHNG |
  ...
  /* Normal mode */
    ...
    p_reg_edmac->EESIPR_b.TCIP =
        1; /* Enable the frame transmit end interrupt. */
  }

  /* Ethernet length 1514bytes + CRC and intergap is 96-bit time */
  p_reg_etherc->RFLR =
      ETHER_ETHERC_RFLR_DEFAULT_VALUE; /* Ethernet length (1514bytes) +
                                          CRC(4byte) */
  p_reg_etherc->IPGR =
      ETHER_ETHERC_IPGR_DEFAULT_VALUE; /* Interpacket Gap is 96-bit time */
  ...
  p_reg_etherc->BCFRR = p_instance_ctrl->p_ether_cfg->broadcast_filter;
     ...
     (defined(__ARMCC_VERSION) && !defined(__ARM_BIG_ENDIAN)) ||               \
     (defined(__ICCARM__) && (__LITTLE_ENDIAN__)))
  ...
  /* Casting the pointer to a uint32_t type is valid because the Renesas
   * Compiler uses 4 bytes per pointer. */
  ...
  if (ETHER_MULTICAST_DISABLE == p_instance_ctrl->p_ether_cfg->multicast) {
    /* Reflect the EESR.RMAF bit status in the RD0.RFS bit in the receive
     * descriptor */
    p_reg_edmac->TRSCER = ETHER_EDMAC_TRSCER_MULTICAST_DISABLE;
  } else {
    /* Don't reflect the EESR.RMAF bit status in the RD0.RFS bit in the receive
     * descriptor */
    p_reg_edmac->TRSCER = ETHER_EDMAC_TRSCER_MULTICAST_ENABLE;
  }

  /* Threshold of Tx_FIFO */
  /* To prevent a transmit underflow, setting the initial value (store and
   * forward modes) is recommended. */
   ...
   * b0      RNR - Receive Request Bit Reset - Continuous reception of multiple
   * frames is possible. b31:b1  Reserved set to 0
   */
 ...
 *                *ptx_pause -
 *                    pointer to location to store the result of the table
 * lookup for transmit PAUSE. 1 is enable, 0 is disable. *prx_pause - pointer to
 * location to store the result of the table lookup for receive PAUSE. 1 is
 * enable, 0 is disable. Return Value : none
 ***********************************************************************************************************************/
static void ether_pause_resolution(uint32_t const local_ability,
                                   uint32_t const partner_ability,
                                   uint32_t *ptx_pause, uint32_t *prx_pause) {
  ...
  for (i = 0; i < ETHER_PAUSE_TABLE_ENTRIES; i++) {
    if ((ability_compare & pause_resolution[i].mask) ==
        pause_resolution[i].value) {
 ...
 * Description  : Software reset is executed, and ETHERC and EDMAC are
 * configured. Arguments    : channel - ETHERC channel number mac_addr - The MAC
 * address of ETHERC mode - The operational mode is specified.
 *                    NO_USE_MAGIC_PACKET_DETECT (0) - Communicate mode usually
 *                    USE_MAGIC_PACKET_DETECT    (1) - Magic packet detection
 * mode Return Value : none
 ***********************************************************************************************************************/
  ...
  link_result =
      p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_api
          ->linkPartnerAbilityGet(
              p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_ctrl,
              &link_speed_duplex, &local_pause_bits, &partner_pause_bits);
  ...
  if (FSP_SUCCESS == link_result) {
    ...
    if (FSP_SUCCESS == err) {
      if (ETHER_NO_USE_MAGIC_PACKET_DETECT == mode) {
        ...
        if ((full_duplex) && (ETHER_FLOW_CONTROL_ENABLE ==
                              p_instance_ctrl->p_ether_cfg->flow_control)) {
          ...
          /* Control of a PAUSE frame whose TIME parameter value is 0 is
           * enabled. */
      ...
      /* At the magic packet detection mode */
        ...
        /* Because data is not transmitted for the magic packet detection
         * waiting, only the reception is permitted. */
        p_reg_etherc->ECMR_b.RE = 1;

        /*
         * The reception function of EDMAC keep invalidity
         * because the receive data don't need to be read when the magic packet
         * detection mode.
         */
      }
    }
...
static uint8_t ether_check_magic_packet_detection_bit(
    ether_instance_ctrl_t const *const p_instance_ctrl) {
  R_ETHERC0_Type *p_reg_etherc =
      (R_ETHERC0_Type *)p_instance_ctrl->p_reg_etherc;
  uint8_t ret = 0;
...
ether_link_status_check(ether_instance_ctrl_t const *const p_instance_ctrl) {
  ...
  link_status =
      p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_api->linkStatusGet(
          p_instance_ctrl->p_ether_cfg->p_ether_phy_instance->p_ctrl);
...
static void ether_call_callback(ether_instance_ctrl_t *p_instance_ctrl,
  ...
  /* Store callback arguments in memory provided by user if available.  This
   * allows callback arguments to be stored in non-secure memory so they can be
   * accessed by a non-secure callback function. */
  ...
  if (!cmse_is_nsfptr(p_instance_ctrl->p_callback)) {
    /* If p_callback is secure, then the project does not need to change
     * security state. */
    p_instance_ctrl->p_callback(p_args);
  } else {
    /* If p_callback is Non-secure, then the project must change to Non-secure
     * state in order to call the callback. */
    ether_prv_ns_callback p_callback =
        (ether_prv_ns_callback)(p_instance_ctrl->p_callback);
    p_callback(p_args);
  }

#else

  /* If the project is not Trustzone Secure, then it will never need to change
   * security state in order to call the callback. */
  p_instance_ctrl->p_callback(p_args);
#endif
 ...
 * Description  : Interrupt handler for Ethernet receive and transmit
 * interrupts. Arguments    : none Return Value : none
 ***********************************************************************************************************************/
void ether_eint_isr(void) {
  ...
  ether_instance_ctrl_t *p_instance_ctrl =
      (ether_instance_ctrl_t *)R_FSP_IsrContextGet(irq);
  ether_extended_cfg_t *p_ether_extended_cfg =
      (ether_extended_cfg_t *)p_instance_ctrl->p_ether_cfg->p_extend;
  ...
  if (status_eesr & ETHER_EDMAC_INTERRUPT_FACTOR_ECI) {
    ...
    if (ETHER_ETHERC_INTERRUPT_FACTOR_LCHNG ==
        (status_ecsr & ETHER_ETHERC_INTERRUPT_FACTOR_LCHNG)) {
    ...
    if (ETHER_ETHERC_INTERRUPT_FACTOR_MPD ==
        (status_ecsr & ETHER_ETHERC_INTERRUPT_FACTOR_MPD)) {
     ...
     */
    p_reg_etherc->ECSR =
        status_ecsr; /* Clear all ETHERC status BFR, PSRTO, LCHNG, MPD, ICD */
  }
  ...
  if (NULL != p_instance_ctrl->p_callback) {
    ...
    for (int i = 0; i < ETHER_EESR_EVENT_NUM; i++) {
      if (status_eesr & ether_eesr_event_mask[i].mask &
          p_ether_extended_cfg->eesr_event_filter) {
    ...
    if (status_eesr & ETHER_EDMAC_INTERRUPT_FACTOR_ECI) {
      for (int i = 0; i < ETHER_ECSR_EVENT_NUM; i++) {
        if (status_ecsr & ether_ecsr_event_mask[i].mask &
            p_ether_extended_cfg->ecsr_event_filter) {
...
ether_configure_padding(ether_instance_ctrl_t *const p_instance_ctrl) {
  R_ETHERC_EDMAC_Type *p_reg_edmac;
  p_reg_edmac = (R_ETHERC_EDMAC_Type *)p_instance_ctrl->p_reg_edmac;
  p_reg_edmac->RPADIR_b.PADR =
      (unsigned)p_instance_ctrl->p_ether_cfg->padding_offset &
      ETHER_MAXIMUM_PADDING_OFFSET;
